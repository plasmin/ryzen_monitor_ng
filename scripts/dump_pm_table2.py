...
def findBenchPath():
    ...
    if program == False:
        print("Unable to find either pigz installed on the system for stress testing. Please install it.")
        exit(6)
...
def main():
    ...
    print("Dumping {:d} instances of the PM table during single thread load ...".format(SAMPLES))
    subprocess.Popen("sh -c \"{0} -p 1 -11 -c -f > /dev/null 2>&1 < /dev/zero\"".format(testerPath), shell=True)
    ...
    print("Dumping {:d} instances of the PM table during 2 threads load ...".format(SAMPLES))
    subprocess.Popen("sh -c \"{0} -p 2 -11 -c -f > /dev/null 2>&1 < /dev/zero\"".format(testerPath), shell=True)
